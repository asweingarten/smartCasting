...
public class mainL : MonoBehaviour {

	public List<string> jsonLog;
	public List<Task> taskList;
	public int currentTaskNo = 0;
	public const int noOfTasks = 19;
	...
	public class TargetingTask : Task {
		...
		public TargetingTask(int taskNo, Vector3 initialPosition) {
			no = taskNo;
			target = GameObject.Find("target" + ((no % 2) == 0 ? "A" : "B"));
			name = "Select sphere";
			...
			if (obj.TryGetValue ("name", out name)) {
				if (name != null) {
					if (name.ToString().Equals("gyro")) {
						...
						gyroX = float.Parse (gamma.ToString());
	...
	void LogObjects() {
		...
		GameObject target = GameObject.Find("targetA");		
		GameObject targetB = GameObject.Find("targetB");		
	...
	void createTasks() {
		...
		for (int i = 1; i < 20; i=i+4) {
			taskList.Add ((TranslatingTask) new TranslatingTask (i, new Vector3(UnityEngine.Random.Range (0.0f,8.0f),UnityEngine.Random.Range (2.0f,5.0f),UnityEngine.Random.Range (4.0f,6.0f)), 
			                                                       new Vector3(UnityEngine.Random.Range (-8.0f,0.0f),UnityEngine.Random.Range (1.0f,6.0f),UnityEngine.Random.Range (1.0f,4.0f))));
...
//		System.IO.File.WriteAllLines("e:/resultsLLL"+UnityEngine.Random.Range(0.0f,1.0f).ToString()+".csv",jsonLog.ToArray());

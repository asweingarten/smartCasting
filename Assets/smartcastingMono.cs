...
public class smartcastingMono : MonoBehaviour {
	...
	private Client client = new Client("http://localhost:3000");//new Client("http://198.61.165.79:8091");//new Client("http://24.246.24.104:8091"); 
	...
	public class TargetingTask : Task {
		...
		public override void start(){
			...
			startT.transform.position = new Vector3(0f,4.5f,0.05f);
			
			occluders [0].transform.position = targetInit - new Vector3 (0f, 0f, 12.0f);
			occluders [1].transform.position = targetInit - new Vector3 (0f, 1.7f, 0f);
			occluders [2].transform.position = targetInit - new Vector3 (1.7f, 0f, 0f);
			occluders [3].transform.position = targetInit + new Vector3 (0f, 0f, 1.7f);
			occluders [4].transform.position = targetInit + new Vector3 (0f, 1.7f, 0f);
			occluders [5].transform.position = targetInit + new Vector3 (1.7f, 0f, 0f);
			
			for (int i = 6; i <=42; i++) {
				float randx = UnityEngine.Random.Range (-9f, 9f);
				...
				while ( Mathf.Abs(randx-targetInit.x) < targetScale +0.25 && Mathf.Abs(randy-targetInit.y) < targetScale +0.25 && Mathf.Abs(randz-targetInit.z) < targetScale +0.25) {
					randx = UnityEngine.Random.Range (-9f, 9f);
					randy = UnityEngine.Random.Range (0.2f, 9f);
					randz = UnityEngine.Random.Range (1f, 14f);
				}
				occluders [i].transform.position = new Vector3 (randx, randy, randz) - new Vector3 (50f, 50f, 0.0f);
			}
			...
			if (obj.TryGetValue ("name", out name)) {
				if (name != null) {
					...
					if (name.ToString().Equals("tapmove")) {
						...
						if ( tempTapY > oldTapY) {
							if (tapY >= 0.133f)
								tapY = tapY - 0.0033f;//(oldTapY > float.Parse (mx.ToString())) ? (tapY + 1f) : (tapY - 1f);//1.0f - float.Parse (my.ToString())+0.02f;
	...
	void Start () {
		...
		jsonLog.Add ("SmarcastingMono,timestamp,object, pitch, yaw, cursor x,cursor y,cursor z, cursor local x, cursor local y, target x, target y, target z, target scale, action");
		// add tasks
		createTasks();
	...
	void LogObjects() {
		...
		if (dragging && (transform.position.x != interactorState.x || transform.position.y != interactorState.y || transform.position.z != interactorState.z)) {
			...
			jsonLog.Add("SmarcastingMono,"+taskList[currentTaskNo].no.ToString() + "," + currentTime + ",interactor," + (360f-transform.parent.transform.rotation.eulerAngles.x).ToString() + "," + (transform.parent.transform.rotation.eulerAngles.y).ToString() + ","  + transform.position.x.ToString() + "," + transform.position.y.ToString() + "," + transform.position.z.ToString() + "," + transform.localPosition.x.ToString()  + "," + transform.localPosition.y.ToString() + " , , , , ,cursor moves");
		}
	...
	void OnCollisionEnter(Collision c)
	{
		...
		if (c.gameObject.name == "target" && startSelected) {
			...
			jsonLog.Add("SmarcastingMono," + taskList[currentTaskNo].no.ToString() + "," + Time.realtimeSinceStartup.ToString()+ ",interactor," + (360f-transform.parent.transform.rotation.eulerAngles.x).ToString() + "," + (transform.parent.transform.rotation.eulerAngles.y).ToString() + "," + transform.position.x.ToString() + "," 
		...
		if (c.gameObject.name == "start") {
			jsonLog.Add ("SmarcastingMono," +taskList [currentTaskNo].no.ToString () + "," + Time.realtimeSinceStartup.ToString () + ",interactor," + (360f - transform.parent.transform.rotation.eulerAngles.x).ToString () +  "," + (transform.parent.transform.rotation.eulerAngles.y).ToString() + ","+ transform.position.x.ToString () + "," + transform.position.y.ToString () + "," + transform.position.z.ToString () + "," + transform.position.z.ToString () + "," + transform.localPosition.x.ToString () + " , , , , ,start entered");
			c.gameObject.renderer.material.color = Color.magenta;
	...
	{
		//if selected, change target and interactor color
		if (c.gameObject.name == "target" && startSelected && selectionTriggered) {
			targetSelectedTime = Time.realtimeSinceStartup-startSelectedTime;
			jsonLog.Add("SmarcastingMono," +taskList[currentTaskNo].no.ToString() + "," + Time.realtimeSinceStartup.ToString()+ ",interactor," + (360f-transform.parent.transform.rotation.eulerAngles.x).ToString() + "," + (transform.parent.transform.rotation.eulerAngles.y).ToString() + ","+ transform.position.x.ToString() + "," 
			            + transform.position.y.ToString() + "," + transform.position.z.ToString()  + "," + transform.localPosition.x.ToString() + "," + transform.localPosition.y.ToString() + "," 
			            + c.gameObject.transform.position.x.ToString() + "," + c.gameObject.transform.position.y.ToString() + "," + c.gameObject.transform.position.z.ToString() +", "+ c.gameObject.transform.localScale.x.ToString()+ ",target selected, ,"+targetSelectedTime.ToString());
			...
			if (((TargetingTask)taskList[currentTaskNo]).targetScale >  0.9f) 
				selectionTimeLarge.Add(targetSelectedTime);
			else 
				selectionTimeSmall.Add(targetSelectedTime);
		...
		if (c.gameObject.name == "target" && startSelected && rotationTriggered) {
			c.gameObject.renderer.material.color = Color.green;
			c.gameObject.transform.localEulerAngles = new Vector3(c.gameObject.transform.localRotation.eulerAngles.x, c.gameObject.transform.localRotation.eulerAngles.y, gyroZ);
			if (((TargetingTask)taskList[currentTaskNo]).targetScale >  0.9f) 
				selectionTimeLarge.Add(targetSelectedTime);
			else 
				selectionTimeSmall.Add(targetSelectedTime);
		...
		if (c.gameObject.name == "start" && selectionTriggered ) {
			...
			startSelected = true;
			jsonLog.Add("SmarcastingMono," +taskList[currentTaskNo].no.ToString() + "," + startSelectedTime+ ",interactor," + (360f-transform.parent.transform.rotation.eulerAngles.x).ToString() + "," + (transform.parent.transform.rotation.eulerAngles.y).ToString() + "," + transform.position.x.ToString() + "," + transform.position.y.ToString() + "," + transform.position.z.ToString() + "," + transform.position.z.ToString()  + "," + transform.localPosition.x.ToString() + " , , , , ,start selected");
			c.gameObject.renderer.material.color = Color.green;
			c.gameObject.transform.position = new Vector3(100f,100f,100f);
		...
		if (c.gameObject.name.StartsWith("occluder") && selectionTriggered) {
			...
			c.gameObject.renderer.material.color = Color.magenta;
			jsonLog.Add("SmarcastingMono," +taskList[currentTaskNo].no.ToString() + "," + Time.realtimeSinceStartup.ToString()+ ","+c.gameObject.name+"," + (360f-transform.parent.transform.rotation.eulerAngles.x).ToString() + "," + (transform.parent.transform.rotation.eulerAngles.y).ToString() + ","+ transform.position.x.ToString() + "," 
			            + transform.position.y.ToString() + "," + transform.position.z.ToString()  + "," + transform.localPosition.x.ToString() + "," + transform.localPosition.y.ToString() + "," 
...
*/
		...
		transform.localPosition = new Vector3(0.0f,-0.3f,0.8f);
		//ray origin position
		...
		transform.parent.transform.rotation = Quaternion.Lerp(transform.rotation, Quaternion.Euler(gyroX+20f, gyroY, 0.0f), damping);
...
*/
	...
	void createTasks() {
		...
		for (int i = 0; i < noOfTasks/2; i++) {
			taskList.Add ((TargetingTask) new TargetingTask (i, 3f, 1.5f, 40));
		}
		for (int i = noOfTasks/2; i < noOfTasks; i++) {
			taskList.Add ((TargetingTask) new TargetingTask (i, 1.5f, 0.75f, 40));
		}
...
//		System.IO.File.WriteAllLines("/Users/kp/Downloads/temp.txt",jsonLog.ToArray());/
//		String content = System.IO.File.ReadAllText ("/Users/kp/Downloads/temp.txt");
//		System.IO.File.AppendAllText ("/Users/kp/Downloads/SmartcastingMono" + sufix + ".csv", content);
		jsonLog.Clear ();
...
//		System.IO.File.AppendAllText("c:/results/calculatedResultsLarge.txt","SmarcastingMono,"+sufix+","+average.ToString()+","+stdev.ToString ()+","+selectionErrorLarge.ToString()+","+selectionTimeLarge.Count+Environment.NewLine);
...
//		System.IO.File.AppendAllText("c:/results/calculatedResultsSmall.txt","SmarcastingMono,"+sufix+","+average.ToString()+","+stdev.ToString ()+","+selectionErrorSmall.ToString ()+","+selectionTimeSmall.Count+Environment.NewLine);
